...
import { Calendar, Clock, FileText, Trash2, Eye, Pencil, Filter } from 'lucide-react';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function TimeLogHistory({ logs, onDelete, onEdit }: TimeLogHistoryProps) {
  ...
  const filteredLogs = useMemo(() => {
    const parsedHours = hoursFilter.trim() === '' ? null : Number(hoursFilter);
    ...
    return sortedLogs.filter((log) => {
      ...
      const matchesHours =
        parsedHours === null ||
        (!Number.isNaN(parsedHours) && Math.abs(log.hoursWorked - parsedHours) < 0.01);

      return matchesMonth && matchesHours;
    });
  }, [hoursFilter, selectedMonth, sortedLogs]);
    ...
    <Card className="overflow-hidden border border-white/70 bg-white/85 shadow-[0_18px_60px_-34px_rgba(15,23,42,0.38)] backdrop-blur-xl dark:border-slate-700/70 dark:bg-slate-900/70 dark:shadow-[0_20px_70px_-35px_rgba(2,6,23,0.9)]">
      ...
      <CardContent className="px-4 pb-5 pt-5 sm:px-6">
        {logs.length === 0 ? (
          ...
          <>
            <div className="mb-4 rounded-[1.25rem] border border-slate-200/80 bg-slate-50/80 p-4 dark:border-slate-700 dark:bg-slate-800/75">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Select value={selectedMonth} onValueChange={setSelectedMonth}>
                    <SelectTrigger className="h-11 rounded-2xl border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800/80">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="e.g. 8"
                    className="h-11 rounded-2xl border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800/80"
                  />
                </div>
              </div>
            </div>
            ...
            {filteredLogs.length === 0 ? (
              <div className="py-12 text-center text-slate-500 dark:text-slate-400">
                ...
                <p className="text-sm">No history entries match the selected filters.</p>
              </div>
            ) : (
